...
class core {
	public function notifBar(){
		...
		if(isset($_SESSION['uid'])){
			...
			echo'</div><div role="tabpanel" class="tab-pane" id="read"><h3>Read</h3>';
		...
		if(isset($operator)){
			...
			if($option === 'sidebar'){
				foreach($modules as $name => $module) if ($module['enabled'] && ($module['admin'] == '0')) {
					echo '<li><a class="btn btn-default width100" href="'.$module['sidebar'].'">'.$module['sidebarDesc'].'</a></li>';
				}
			}
			if($option === 'acp'){
				foreach($modules as $name => $module) if ($module['enabled'] && ($module['admin'] == '1')) {
					if($this->verify("core.*") || $this->verify($module['perms']))
					echo '<li><a class="btn btn-default width100" href="'.$module['acp'].'">'.$module['sidebarDesc'].'</a></li>';
				}
			}
		}
	}
